...
async fn test_integrated_hardlink_copying() {
    ...
    writeln!(file, "This is the original content that will be hardlinked.").expect("Failed to write to original file");
    drop(file);
    ...
    writeln!(file, "This is a regular file, not a hardlink.").expect("Failed to write to regular file");
    drop(file);
    ...
    assert!(stats.files_copied >= 4, "Should have copied at least 4 files");
    assert!(stats.directories_created >= 1, "Should have created at least 1 directory");
    assert!(stats.errors == 0, "Should have no errors");
    
    // Verify that all files exist in destination
    assert!(dst_path.join("original.txt").exists(), "Original file should exist");
    assert!(dst_path.join("hardlink1.txt").exists(), "Hardlink1 should exist");
    assert!(dst_path.join("hardlink2.txt").exists(), "Hardlink2 should exist");
    assert!(dst_path.join("regular.txt").exists(), "Regular file should exist");
    assert!(dst_path.join("subdir").exists(), "Subdirectory should exist");
    assert!(dst_path.join("subdir").join("subdir_file.txt").exists(), "Subdir file should exist");
    
    // Verify that hardlinks have the same content
    let original_content = std::fs::read_to_string(dst_path.join("original.txt")).expect("Failed to read original");
    let hardlink1_content = std::fs::read_to_string(dst_path.join("hardlink1.txt")).expect("Failed to read hardlink1");
    let hardlink2_content = std::fs::read_to_string(dst_path.join("hardlink2.txt")).expect("Failed to read hardlink2");
    
    assert_eq!(original_content, hardlink1_content, "Hardlink1 should have same content as original");
    assert_eq!(original_content, hardlink2_content, "Hardlink2 should have same content as original");
    assert_eq!(hardlink1_content, hardlink2_content, "Hardlinks should have same content");
    
    // Verify that regular file has different content
    let regular_content = std::fs::read_to_string(dst_path.join("regular.txt")).expect("Failed to read regular");
    assert_ne!(original_content, regular_content, "Regular file should have different content");
}
...
async fn test_filesystem_boundary_detection() {
    ...
    assert!(stats.errors == 0, "Should have no filesystem boundary errors");
    assert!(stats.files_copied >= 1, "Should have copied at least 1 file");
    
    // Verify file was copied
    assert!(dst_path.join("test.txt").exists(), "Test file should exist in destination");
}
...
async fn test_symlink_handling() {
    ...
    assert!(stats.symlinks_processed >= 1, "Should have processed at least 1 symlink");
    
    // Verify symlink was copied
    assert!(dst_path.join("symlink.txt").is_symlink(), "Symlink should exist in destination");
    assert!(dst_path.join("target.txt").exists(), "Target file should exist in destination");
}
...
async fn test_filesystem_tracker_functionality() {
    ...
    assert!(initial_stats.source_filesystem.is_none(), "Source filesystem should be None initially");
    
    // Set source filesystem
    tracker.set_source_filesystem(12345);
    let updated_stats = tracker.get_stats();
    assert_eq!(updated_stats.source_filesystem, Some(12345), "Source filesystem should be set");
    
    // Test filesystem boundary detection
    assert!(tracker.is_same_filesystem(12345), "Same filesystem should return true");
    assert!(!tracker.is_same_filesystem(54321), "Different filesystem should return false");
    ...
    // Test inode tracking
    assert!(!tracker.is_inode_copied(100), "Inode should not be marked as copied initially");
    
    // Mark inode as copied
    tracker.mark_inode_copied(100, std::path::Path::new("/dst/file.txt"));
    assert!(tracker.is_inode_copied(100), "Inode should be marked as copied");
    
    // Test getting original path
    let original_path = tracker.get_original_path_for_inode(100);
    assert!(original_path.is_some(), "Should be able to get original path");
    assert_eq!(original_path.unwrap(), std::path::Path::new("/dst/file.txt"));
