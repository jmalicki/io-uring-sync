...
pub async fn copy_directory(
    ...
    info!("Starting directory copy from {} to {}", src.display(), dst.display());
    
    // Create destination directory if it doesn't exist
    if !dst.exists() {
        tokio_fs::create_dir_all(dst).await.map_err(|e| {
            SyncError::FileSystem(format!("Failed to create destination directory {}: {}", dst.display(), e))
        })?;
...
async fn traverse_and_copy_directory(
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| {
        ...
        let metadata = entry.metadata().await.map_err(|e| {
            SyncError::FileSystem(format!("Failed to get metadata for {}: {}", src_path.display(), e))
        })?;
        ...
        if metadata.is_dir() {
            ...
            if !dst_path.exists() {
                tokio_fs::create_dir(&dst_path).await.map_err(|e| {
                    SyncError::FileSystem(format!("Failed to create directory {}: {}", dst_path.display(), e))
                })?;
                stats.directories_created += 1;
            }
            
            // Recursively process subdirectory
            if let Err(e) = traverse_and_copy_directory(&src_path, &dst_path, file_ops, copy_method.clone(), stats).await {
            ...
            match copy_file(&src_path, &dst_path, copy_method.clone()).await {
                Ok(()) => {
                    ...
                    if let Err(e) = preserve_file_metadata(&src_path, &dst_path, file_ops).await {
                        warn!("Failed to preserve metadata for {}: {}", dst_path.display(), e);
                    }
                }
                Err(e) => {
...
async fn copy_symlink(src: &Path, dst: &Path) -> Result<()> {
    let target = tokio_fs::read_link(src).await.map_err(|e| {
        SyncError::FileSystem(format!("Failed to read symlink target for {}: {}", src.display(), e))
    })?;
    
    // Remove destination if it exists
    if dst.exists() {
        tokio_fs::remove_file(dst).await.map_err(|e| {
            SyncError::FileSystem(format!("Failed to remove existing destination {}: {}", dst.display(), e))
        })?;
    }
    ...
    #[cfg(unix)]
    {
        tokio_fs::symlink(&target, dst).await.map_err(|e| {
            SyncError::FileSystem(format!("Failed to create symlink {} -> {}: {}", dst.display(), target.display(), e))
        })?;
    }
    
    #[cfg(not(unix))]
    {
        return Err(SyncError::FileSystem("Symlink creation not supported on this platform".to_string()));
    }
...
async fn preserve_file_metadata(
    ...
    let metadata = file_ops.get_file_metadata(src).await.map_err(|e| {
        SyncError::FileSystem(format!("Failed to get source metadata for {}: {}", src.display(), e))
    })?;
    
    // Set permissions
    file_ops.set_file_permissions(dst, metadata.permissions).await.map_err(|e| {
        SyncError::FileSystem(format!("Failed to set permissions for {}: {}", dst.display(), e))
    })?;
    
    // Set ownership
    file_ops.set_file_ownership(dst, metadata.uid, metadata.gid).await.map_err(|e| {
        SyncError::FileSystem(format!("Failed to set ownership for {}: {}", dst.display(), e))
    })?;
...
pub async fn get_directory_size(path: &Path) -> Result<u64> {
    ...
    let mut entries = tokio_fs::read_dir(path).await.map_err(|e| {
        SyncError::FileSystem(format!("Failed to read directory {}: {}", path.display(), e))
    })?;
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| {
        ...
        let metadata = entry.metadata().await.map_err(|e| {
            SyncError::FileSystem(format!("Failed to get metadata for {}: {}", entry_path.display(), e))
        })?;
