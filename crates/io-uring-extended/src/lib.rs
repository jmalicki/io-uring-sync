...
impl ExtendedRio {
    ...
    ///
    /// This wraps a rio instance for extended operations.
    ...
    /// Copy a range of bytes from one file to another using `copy_file_range` syscall.
    ///
    /// This is an in-kernel copy operation that is highly efficient.
    ///
    /// # Parameters
    ///
    ...
    pub async fn copy_file_range(
        ...
        // Use direct syscall implementation
    ...
    /// Get extended attribute value for a given path.
    ///
    /// # Parameters
    ///
    ...
    /// Synchronous getxattr implementation
    fn getxattr_syscall(
        &self,
        path: &std::path::Path,
        name: &str,
        ...
        let name_c = std::ffi::CString::new(name.as_bytes())
            .map_err(|e| ExtendedError::NotSupported(format!("Invalid xattr name: {}", e)))?;
    ...
    /// Set extended attribute value for a given path.
    ///
    /// # Parameters
    ///
    /// * `path` - Path to the file or directory
    ...
    fn setxattr_syscall(&self, path: &std::path::Path, name: &str, value: &[u8]) -> Result<()> {
        ...
        let result = unsafe {
            libc::setxattr(
                ...
                value.len(),
                0, // Flags (e.g., XATTR_REPLACE, XATTR_CREATE)
            )
        };
    ...
    ///
    /// * `path` - Path to the file or directory
    /// * `buffer` - Buffer to store the attribute names (null-separated)
    ///
    /// # Returns
    ///
    ...
    /// # Parameters
    ///
    /// * `_dir_fd` - Directory file descriptor.
    /// * `_buffer` - Buffer to store directory entries.
    ///
    /// # Returns
    ///
    /// Returns the number of bytes read or an error.
